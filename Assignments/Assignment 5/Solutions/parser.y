...
| IF                                             { fprintf(output,"if%d:  ", lblcount); nestingstack[nestinglevel++] = lblcount; }
comparison
THEN                                              { fprintf(output,"tst r0\nbeq eif%d\n",lblcount);   lblcount++;}
statements ENDIF SEMICOLON                   { int lbl; nestinglevel--; lbl = nestingstack[nestinglevel]; fprintf(output,"eif%d: ",lbl);};
